...
class WPF_Public
{
	...
	private $custom_cols;
	private $main_query_key = 'themify_wpf_query';
		...
		if ( !empty( $query_args ) ) {
            ...
            if(false !== $main_query_args){
                foreach ($main_query_args as $arg_key=>$arg_val){
                    if(!empty($main_query_args[$arg_key]) && empty($query_args[$arg_key])){
	...
	public function parse_query( array $post, array $form, $build = true ) {
		...
		if ( $build ) {
			...
			if ( !$this->append && !empty( $post['orderby'] ) ) {
